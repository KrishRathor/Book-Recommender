...
import pandas as pd
import numpy as np
from MlModel import final_rating, recommend_book, books

book_image = pd.read_csv('books.csv', sep=';', error_bad_lines=False, encoding='latin-1')
...
def recommend_page():
    ...
    for i in range(5):
       authors.append(get_specified_column(books, 'title', recommend_book_list[0][i+1], 'author'))
       image_url.append(get_specified_column(book_image, 'title', recommend_book_list[0][i], 'image'))
       
    print(f"list of author ifidjfi j  jiejei{authors}")
